...
import post_process_modified as pp
...
# Required parameters
flags.DEFINE_string(
    "bert_config_file", 'bert-config-vocab/bert_config.json',
    "The config json file corresponding to the pre-trained BERT model. "
    "This specifies the model architecture.")

flags.DEFINE_string("vocab_file", 'bert-config-vocab/vocab-nq.txt',
...
flags.DEFINE_string(
    "predict_file", 'input/nq-dev-sample.jsonl.gz',
...
flags.DEFINE_string(
    "final_output_prediction_file", None,
    "Where to print predictions in NQ prediction format, to be passed to"
    "natural_questions.nq_eval.")
...
def compute_pred_dict(candidates_dict, dev_features, raw_results):
    ...
    nq_pred_dict = {}
    for e in examples:
        nbest_summary = compute_predictions(e)
...
def main():
    ...
    eval_set = 'nq_model/output/eval'   # tag eval set and define path for tf_record output
    ...
    # Dump predictions to pickle file
    ...
    nbest_ens_list.append(nbest_summary_dict)
...
if __name__ == "__main__":
    ...
    # Define checkpoints for models in ensemble
    ckpts = ['nq_model/model-a/wwm_stride_192/model.ckpt-7000',
             'nq_model/model-b/wwm_maxcont_200/model.ckpt-9500',
             'nq_model/model-c/wwm_cased/model.ckpt-8500'
             ]
    # Create list of combined prediction models for all models
    pred_files = [x.split('/')[-2] for x in ckpts]

    nbest_ens_list = []

    # Define and read in evaluation examples
    eval_examples = read_nq_examples(
                input_file=FLAGS.predict_file, is_training=False)

    FLAGS.do_lower_case = True
    FLAGS.vocab_file = 'nq_model/bert-config-vocab/vocab-nq.txt'
    FLAGS.bert_config_file = 'nq_model/bert-config-vocab/bert_config.json'
    FLAGS.output_dir = 'nq_model/output'
    for i in range(len(ckpts)-1):
        FLAGS.init_checkpoint = ckpts[i]
        FLAGS.output_prediction_file =  'nq_model/output/' + pred_files[i] + '.json'
        main()

    FLAGS.do_lower_case = False
    FLAGS.vocab_file = 'nq_model/bert-config-vocab/vocab_cased-nq.txt'
    FLAGS.bert_config_file = 'nq_model/bert-config-vocab/bert_config_cased.json'
    i = 2
    FLAGS.init_checkpoint = ckpts[i]
    FLAGS.output_prediction_file = 'nq_model/output/' + pred_files[i] + '.json'
    main()
    ...
    final_nq_pred_dict = pp.get_nq_pred_dict(nbest_ens_list)

    final_predictions_json = {"predictions": list(final_nq_pred_dict)}  # format dictionary for JSON conversion

    with tf.gfile.Open(FLAGS.final_output_prediction_file, "w") as f:
        json.dump(final_predictions_json, f, indent=4)

    print('Total time elapsed: {} seconds'.format(time.time()-time_start))